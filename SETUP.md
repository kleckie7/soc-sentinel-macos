Markdown# How to Build **SOC Sentinel** – macOS-native Threat Detection Tool  
...
```bash
# Install or update Homebrew (skip if already installed)
/bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)"
Bash# Install Python 3.13 + GitHub CLI
brew install python@3.13 gh
Bash# Log in to GitHub (one-time – opens browser)
gh auth login
...
Bash# 2. Set up Python environment
python3 -m venv venv
source venv/bin/activate
pip install --upgrade pip
pip install pandas==2.2.3 matplotlib==3.9.2
pip freeze > requirements.txt
Bash# 3. Create the tool (main.py) – this is the exact working version
cat > main.py <<'EOF'
#!/usr/bin/env python3
...
chmod +x main.py
mkdir -p reports
Bash# 4. Test it (must work 100%)
./main.py
# → A stunning dark-mode report opens automatically in Preview
Bash# 5. Create perfect README.md
cat > README.md <<'EOF'
